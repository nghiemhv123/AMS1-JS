...
const bmiBtn = document.getElementById("indexbmi-btn");
...
function Validate() {
    ...
    } else if (ageInput.value == "" || ageInput.value > 15) {
        alert("Age must be between 1 and 15!");
    } else if (typeInput.value == "Select Type") {
        alert("Please select Type!");
    } else if (weightInput.value == '' || weightInput.value > 15) {
        alert("Weight must be between 1 and 15!");
    } else if (lengthInput.value == "" || lengthInput.value > 100) {
...
function renderTableData(petArr) {
    ...
    <td >${petArr[i].bmi}</td>
    <td style="color:blue;" >${petArr[i].Date.toLocaleDateString('vi-VN', {
            hour12: false,
        })}</td>
...
const indexBMI = () => {
    for (let i = 0; i < petArr.length; i++) {
        ...
        //toFixed(x)làm tròn một số đến chữ số thập phân thứ x
    }
    renderTableData(petArr)
}